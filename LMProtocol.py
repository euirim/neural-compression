...
class LMProtocol:
    ...
    def _get_binary_from_object(self, compressed_object):
        ...
        binary_initial_context.extend(
            ("{0:0" + str(math.log2(self._initial_context_max_bit_size)) + "b}").format(
                str(len(initial_context_bytes))
            )
        )
        ...
        for word in compressed_object["words"]:
            ...
            if word["out_of_vocabulary"]:
                ...
                binary_word.extend(
                    (
                        "{0:0"
                        + str(math.log2(self._out_of_vocabulary_word_max_bit_size))
                        + "b}"
                    ).format(str(len(uncompressed_word)))
                )
                ...
                binary_word.extend(
                    (
                        "{0:0"
                        + str(math.log2(self._next_word_possibilities_number))
                        + "b}"
                    ).format(str(word["ranking"]))
                )
        ...
        while len(words_binary) > 0:
            ...
            if word["out_of_vocabulary"]:
                word_binary_length_binary = words_binary[
                    1 : 1 + int(math.log2(self._out_of_vocabulary_word_max_bit_size))
                ]
